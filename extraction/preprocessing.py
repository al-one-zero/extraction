...
def convert_emojis(text):
    for emot in UNICODE_EMO:
        text = text.replace(
            emot,
            "("
            + (UNICODE_EMO[emot].replace(",", "").replace(":", "").replace("_", " "))
            + ")",
        )
    return text
...
def preprocess_tweet(tweet):
    return (
        convert_emojis(
            replace_links(remove_slash_n(remove_hashtags_and_mentions(tweet)))
        )
        if tweet != None
        else None
    )
...
def read_txt_file(path:str ="../data/train.txt") -> pd.DataFrame:
    ...
    matrix = np.array([re.findall(exp, l) for l in open(path, "r").readlines()]).reshape(-1, 4)
    return pd.DataFrame(matrix[:, 1:], columns=column_names, index=matrix[:, 0].astype(np.int32))
...
def preprocess_dataset(df_, fasttext_model_location="../data/lid.176.bin"):
    ...
    df["Language"] = df.Tweet.apply(
        lambda x: model.predict(tweet_to_lower(x), k=1)[0][0][len("__label__") :]
    )
    df["LanguageProbability"] = df.Tweet.apply(
        lambda x: model.predict(tweet_to_lower(x), k=1)[1][0]
    )
